...
async function register(req,res){
    ...
    if(!username||!firstname||!lastname||!email||!password){
        return res.status(401).json({msg:"pleas inter all requierd information"})
    }
    try {
        ...
        if(password.length<8){
            return res.status(401).json({msg:"The passord is too short it must be at least 8 characters"})
        }
...
try {
    ...
    if(user.length==0){
        return res.status(401).json({msg:"Creat account first"})
    }
    //compare passored
